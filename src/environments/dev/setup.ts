...
export const setupDevEnvironment = async (dockerImages: { apiImage: awsx.ecs.Image; webappImage: awsx.ecs.Image; }) => {
    ...
    dbCluster.endpoint.apply(async (dbHost: any) => {
      ...
      backendAPIResponse.backendAPIListener.endpoint.hostname.apply(async (apiUrl: string) => {                       
        console.log(`Create API CNAME: ${apiUrl} -> ${config.devApiDomain}`);
        console.log(`API will be available on: ${config.fullDevApiUrl}`);
        
        const frontendResponse = await frontend.createFrontend(dockerImages.webappImage, cluster, config.fullDevApiUrl);
  
        frontendResponse.frontendListener.endpoint.hostname.apply(async (frontendUrl: string) => {
          console.log(`Create Web App CNAME: ${frontendUrl} -> ${config.devWebappDomain}`);                    
          console.log(`Web App will be available on: ${config.fullDevWebappUrl}`);
        });
      });
